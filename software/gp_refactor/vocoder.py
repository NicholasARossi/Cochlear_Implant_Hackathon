...
from software.gp_refactor.vocoderTools import ElFieldToActivity, ActivityToPower, NeurToBinMatrix, generate_cfs
        ...
        if electrodogram[-3:] == '.h5':
            with h5py.File(electrodogram, 'r') as f:
                if len(list(f.keys())) == 1:
                    ...
                    raise ValueError(
                        'HDF5 File contains multiple datasets. File should contain only the electrode pulse matrix.')
                f.close()
