...
from software.gp_refactor.classes import FitnessWrapper
...
class Audit:
    def __init__(
        ...
        self.toolbox, self.mstats, self.fw = custom_toolbox(self.wavefile_path,primitives_list=['convolution','noise','multiplication','filter','phase','noise','scale','clip'])

        # fw = FitnessWrapper(self.wavefile_path)
    ...
    def _mega_audio_out(self):


        mega_out_folder=os.path.join(self.figure_folder,'mega_out')

        if not os.path.exists(mega_out_folder):
            os.makedirs(mega_out_folder)
        ...
        for l, checkpoint in enumerate(self.checkpoints):
            ...
            best_score=0
            best_ind=[]
            for i,ind in enumerate(results['population']):
                # values = ind.fitness.values
                # if sum(values)>best_score:
                #     best_score=np.sum(values)
                #     best_ind=ind
                ...
                name=checkpoint.split('/')[-1].split('.')[0]
                write_wav(f'{mega_out_folder}/{name}_{i}.wav', self.fw.audioFs,
                          self.fw.audioOut)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-c', '--checkpoint_folder', dest="checkpoint_folder", type=str, default='../../archived_results/words_1d_hill',
